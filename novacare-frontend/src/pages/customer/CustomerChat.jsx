...
export default function CustomerChat() {
  ...
  useEffect(() => {
        ...
        receiveHandler = (rawMsg) => {
          ...
          if (String(msg.conversationId) === String(activeConvIdRef.current)) {
            ...
            window.dispatchEvent(
              new CustomEvent("novachat-read", { detail: { delta: 0 } }),
            );
          }
        };
    ...
    return () => {
      ...
      if (conn) {
        if (receiveHandler) conn.off("ReceiveMessage", receiveHandler);
        if (newConvHandler) conn.off("NewConversation", newConvHandler);
      }
    };
  ...
  const openConversation = useCallback(
    (conv) => {
      ...
      if (unread > 0) {
        ...
        window.dispatchEvent(
          new CustomEvent("novachat-read", { detail: { delta: unread } }),
        );
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // Server will echo back via ReceiveMessage with real id — replace optimistic
    } catch {
    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "var(--text-muted)" }}>
          Chat with our pharmacy team in real time
        </p>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: activeConvId ? "280px 1fr" : "1fr",
          gap: 20,
          height: 580,
        }}
      >
                      ...
                      >
                        <span
                          style={{
                            ...
                            fontSize: "0.75rem",
                            fontWeight: 700,
                            background: `${ROLE_COLORS[conv.targetRole]}15`,
                            color: ROLE_COLORS[conv.targetRole],
                          }}
                        >
          ...
          >
            ...
            >
              {messages.map((msg, idx) => {
                // Use senderId comparison (consistent with dedup logic and Chats.jsx)
                const isMe =
                  String(getSenderId(msg)) === String(currentUser?.id);
                return (
                  <div
      ...
      {showRoleModal && (
        ...
        >
          ...
          >
            ...
            >
              Choose a branch and the team you'd like to reach
            </p>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {Object.entries(ROLE_LABELS).map(([role, label]) => (
                  <button
                    key={role}
                    onClick={() => handleStartConversation(Number(role))}
                    disabled={!selectedBranchId}
                    style={{
                      padding: "12px 16px",
                      borderRadius: 10,
                      border: `2px solid ${ROLE_COLORS[role]}25`,
                      ...
                      fontSize: "0.92rem",
                      opacity: selectedBranchId ? 1 : 0.6,
                      transition: "all 0.15s",
                    }}
                    onMouseOver={(e) => {
                      if (selectedBranchId) {
                        e.currentTarget.style.borderColor = ROLE_COLORS[role];
                        e.currentTarget.style.background = `${ROLE_COLORS[role]}15`;
                      }
                    }}
                    onMouseOut={(e) => {
                      e.currentTarget.style.borderColor = `${ROLE_COLORS[role]}25`;
                      e.currentTarget.style.background = selectedBranchId
                        ? `${ROLE_COLORS[role]}08`
                        : "#f1f5f9";
                    }}
                  >
