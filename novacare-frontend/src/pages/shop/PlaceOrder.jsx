...
export default function PlaceOrder() {
  ...
  useEffect(() => {
    getBranches()
      .then(data => {
        ...
        setBranches(active);
        if (active.length > 0) setForm(f => ({ ...f, branchId: String(active[0].id) }));
      })
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rxWithPrescriptions = cartItems.filter(i => i.prescriptionDataUrl && i.requiresPrescription);
      if (rxWithPrescriptions.length > 0 && Array.isArray(created.items)) {
        await Promise.all(rxWithPrescriptions.map(async (cartItem) => {
          const orderItem = created.items.find(i => i.medicineId === cartItem.medicineId);
          ...
          try {
            ...
            console.error('Prescription upload failed for', cartItem.name, uploadErr);
          }
        }));
      }
    ...
    return (
      <div className="shop-section">
        <div className="success-page">
          ...
          <p>Your order <strong>#{orderId}</strong> has been submitted and is awaiting approval.</p>
          <p className="text-muted">You will receive a confirmation once staff review your order.</p>
          ...
          <div className="card-body">
            {missingPrescriptions.length > 0 && (
              <div className="alert alert-warning" style={{ marginBottom: 20 }}>
                ...
                <strong>Prescription required:</strong> Please go back and upload a prescription for:{' '}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" required placeholder="+250 7XX XXX XXX" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                </div>
              </div>
              <div className="form-group">
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  {rxItems.map(item => (
                    <div key={item.medicineId} style={{
                      ...
                      background: item.prescriptionDataUrl ? 'var(--success-bg)' : 'var(--danger-bg)',
                      border: `1px solid ${item.prescriptionDataUrl ? 'var(--success)' : 'var(--danger)'}`,
                    }}>
                      <FileImage size={18} style={{ color: item.prescriptionDataUrl ? 'var(--success)' : 'var(--danger)', flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{item.name}</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                          {item.prescriptionDataUrl ? `✓ ${item.prescriptionName || 'Prescription uploaded'}` : '✗ No prescription — go back to upload'}
                        </div>
                      </div>
                      {item.prescriptionDataUrl && !item.prescriptionName?.endsWith('.pdf') && (
                        <img src={item.prescriptionDataUrl} alt="rx" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 6 }} />
                      )}
                    </div>
                  ))}
                </div>
              ...
              <button type="submit" className="btn btn-primary btn-lg" style={{ width: '100%' }} disabled={missingPrescriptions.length > 0 || submitting}>
                {submitting ? 'Placing Order...' : `Place Order — RWF ${Math.round(total).toLocaleString()}`}
              </button>
            </form>
          </div>
        </div>
          ...
          {cartItems.map(item => (
            <div key={item.medicineId} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border-color)', fontSize: '0.9rem' }}>
              <div>
                ...
                <div className="text-muted">Qty: {item.quantity}{item.requiresPrescription ? ' · Rx' : ''}</div>
              </div>
              <div style={{ fontWeight: 600 }}>RWF {Math.round(item.price * item.quantity).toLocaleString()}</div>
            </div>
          ))}
