...
function PrescriptionModal({ order, item, onClose }) {
  ...
  const fileName =
    item?.prescriptionFileName || item?.prescriptionName || "prescription";
...
export default function Orders() {
  ...
  const handlePrescriptionUpload = async (e) => {
    ...
    e.target.value = "";
    try {
      await uploadPrescriptionForItem(
        uploadingItem.orderId,
        uploadingItem.itemId,
        file,
      );
  ...
  const getPrescriptionItems = (order) =>
    (order.items || []).filter(
      (i) => i.prescriptionImagePath || i.prescriptionDataUrl,
    );
  const getRxItems = (order) =>
    (order.items || []).filter((i) => i.medicine?.requiresPrescription);
  ...
  return (
    <div>
      ...
      {prescriptionModal && (
        <PrescriptionModal
          order={prescriptionModal.order}
          item={prescriptionModal.item}
                                ...
                                {rxItems.map((item, idx) => {
                                  ...
                                  return (
                                    <div
                                      key={idx}
                                      onClick={() =>
                                        setPrescriptionModal({ order, item })
                                      }
                                      style={{
                                        border: "1px solid var(--border-color)",
                                        borderRadius: 10,
