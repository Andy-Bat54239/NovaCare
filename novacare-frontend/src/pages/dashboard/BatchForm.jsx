...
export default function BatchForm() {
  ...
  useEffect(() => {
    Promise.all([getMedicines(), getBranches()])
      .then(([meds, brs]) => {
        setMedicines(meds || []);
        setBranches((brs || []).filter(b => b.isActive));
      })
      .catch(console.error);
  }, []);
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="e.g. Cipla Ltd" value={form.supplier} onChange={e => setForm({ ...form, supplier: e.target.value })} />
            </div>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Quantity</label>
                <input className="form-input" type="number" min="1" required value={form.initialQuantity} onChange={e => setForm({ ...form, initialQuantity: e.target.value })} />
              </div>
              <div className="form-group">
                <label className="form-label">Cost Price (RWF)</label>
                <input className="form-input" type="number" step="100" min="0" required value={form.costPrice} onChange={e => setForm({ ...form, costPrice: e.target.value })} />
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
