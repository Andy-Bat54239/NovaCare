...
function AdminDashboard({
  ...
  const usersByRole = [
    {
      name: "Admin",
      value: allUsers.filter((u) => u.role === 1 && u.isActive).length,
    },
    {
      name: "Manager",
      value: allUsers.filter((u) => u.role === 2 && u.isActive).length,
    },
    {
      name: "Pharmacist",
      value: allUsers.filter((u) => u.role === 3 && u.isActive).length,
    },
  ].filter((d) => d.value > 0);
  ...
  return (
    <>
      ...
      >
        <StatCard
          icon={DollarSign}
          value={`RWF ${Math.round(stats.totalRevenue || 0).toLocaleString()}`}
          label="Total Revenue"
          color="#22c55e"
        />
        <StatCard
        ...
        <div className="card">
          ...
          >
            ...
            <QuickAction
              ...
              description={`${stats.unreadMessages || 0} unread messages`}
            ...
            <QuickAction
              ...
              description="Configure pharmacy system preferences"
        ...
        <div className="card">
          <div className="card-header">
            <SectionHeader
              ...
              onAction={() => navigate("/dashboard/sales")}
            />
          </div>
                  ...
                  {recentSales.slice(0, 5).map((sale) => (
                    <tr key={sale.id}>
                      ...
                      <td style={{ fontWeight: 600, color: "var(--primary)" }}>
                        RWF {Math.round(sale.totalAmount).toLocaleString()}
                      </td>
                      <td>
                ...
                <tbody>
                  {allOrders.slice(0, 5).map((order) => (
                    <tr key={order.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${order.status === "Completed" ? "badge-success" : order.status === "Pending" ? "badge-warning" : order.status === "Approved" ? "badge-primary" : "badge-danger"}`}
                        >
                          {order.status}
                        </span>
                      </td>
...
function ManagerDashboard({
  ...
  const lowStockBatches = branchBatches.filter(
    (b) => b.remainingQuantity < 10 && b.remainingQuantity > 0,
  );
  ...
  return (
    <>
      ...
      >
        <StatCard
          icon={DollarSign}
          value={`RWF ${Math.round(stats.branchRevenue || 0).toLocaleString()}`}
          label="Branch Revenue"
          color="#22c55e"
        />
        <StatCard
          icon={ShoppingCart}
          value={stats.branchSales || 0}
          label="Branch Sales"
          color="#0d9488"
        />
        <StatCard
      ...
      <div className="charts-grid">
        <div className="card">
          ...
          >
            ...
            <QuickAction
              icon={ClipboardList}
              label="Review Orders"
              description={`${stats.branchOrders || 0} orders awaiting approval`}
            ...
            <QuickAction
              ...
              description="View medicine stock and batches"
        ...
        <div className="card">
          <div className="card-header">
            <h3>Branch Team ({branchTeam.length} active)</h3>
          </div>
            ...
            {lowStockBatches.length === 0 ? (
              ...
              lowStockBatches.slice(0, 5).map((batch) => (
                ...
                >
                  <div>
                    <div style={{ fontWeight: 600, fontSize: "0.9rem" }}>
                      {batch.medicine?.brandName || `Med #${batch.medicineId}`}
                    </div>
                    <div
                      style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}
                    >
  ...
  return (
    <>
      <div className="stats-grid">
        <StatCard
          icon={DollarSign}
          value={`RWF ${Math.round(stats.myTodayRevenue || 0).toLocaleString()}`}
          label="My Sales Today"
          color="#22c55e"
          subtitle={`${stats.myTodaySales || 0} transactions`}
        />
        <StatCard
          icon={Target}
          value={`RWF ${Math.round(stats.myTotalRevenue || 0).toLocaleString()}`}
          label="My Total Revenue"
          color="#0d9488"
        />
        <StatCard
          icon={ShoppingCart}
          value={stats.myTotalSales || 0}
        ...
        <div className="card">
          ...
          >
            ...
            <QuickAction
              ...
              description="Look up medicine details and stock"
            ...
            <QuickAction
              icon={Package}
              label="View Batches"
              description="Check batch inventory and expiry dates"
              onClick={() => navigate("/dashboard/batches")}
              color="#6366f1"
            />
            <QuickAction
              icon={ClipboardList}
              label="View Orders"
              description="See incoming customer orders"
              onClick={() => navigate("/dashboard/orders")}
              color="#f59e0b"
            />
          </div>
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      if (currentUser.role === 1) {
        ...
        setRecentSales(
          (salesData || []).filter((s) => s.userId === currentUser.id),
        );
      }
    } catch (err) {
        ...
        <div>
          ...
          <p className="page-header-subtitle">
            ...
            {roleSubtitle[roleName] ||
              "Here's what's happening at NovaCare today"}
          </p>
        </div>
