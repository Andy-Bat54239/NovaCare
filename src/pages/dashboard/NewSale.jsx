...
import { Search, Plus, Minus, Trash2, ShoppingCart, CheckCircle, RefreshCw } from 'lucide-react';
...
export default function NewSale() {
  ...
  const [branchId, setBranchId] = useState(String(currentUser?.branchId || '1'));
  ...
  const medsWithStock = useMemo(() =>
    medicines.map(m => ({
      ...m,
      stock: batches
        .filter(b => b.medicineId === m.id && b.branchId === Number(branchId) && b.remainingQuantity > 0 && new Date(b.expiryDate) > new Date())
  ...
  const addToCart = (med) => {
    setCart(prev => {
      ...
      if (exists) {
        return exists.quantity >= maxQty ? prev :
          prev.map(i => i.medicineId === med.id ? { ...i, quantity: i.quantity + 1 } : i);
      }
      return [...prev, { medicineId: med.id, name: med.brandName, unitPrice: med.price, quantity: 1, maxStock: maxQty }];
    });
  };

  const updateQty = (medId, delta) => {
    setCart(prev => prev.map(i => i.medicineId === medId
      ? { ...i, quantity: Math.min(i.maxStock, Math.max(1, i.quantity + delta)) }
      : i
    ));
  };
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 60, flexDirection: 'column', gap: 16 }}>
        ...
        <p style={{ color: 'var(--text-muted)', margin: 0 }}>Loading POS data...</p>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="success-page">
        ...
        <p>Invoice <strong>{success.invoiceNumber}</strong> has been created successfully.</p>
        <p style={{ fontWeight: 700, fontSize: '1.5rem', color: 'var(--primary)' }}>Total: RWF {Math.round(total).toLocaleString()}</p>
  ...
  return (
    <div>
      <div className="page-header">
        <h1>New Sale (POS)</h1>
        <select className="filter-select" value={branchId} onChange={e => { setBranchId(e.target.value); setCart([]); }}>
          {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
        </select>
      </div>

      {error && (
        <div style={{ background: '#fef2f2', color: '#dc2626', padding: '12px 16px', borderRadius: 8, marginBottom: 16 }}>{error}</div>
      )}
      ...
      <div className="pos-layout">
            ...
            {filtered.length === 0 ? (
              ...
              <div key={m.id} className="medicine-mini-card" onClick={() => addToCart(m)}>
                ...
                <div className="mini-card-footer">
                  <span className="price">RWF {Math.round(m.price).toLocaleString()}</span>
        ...
        <div className="pos-cart">
            ...
            {cart.length === 0 ? (
              ...
              <div key={item.medicineId} className="pos-cart-item">
                <div className="pos-cart-item-info">
                  ...
                  <div className="pos-cart-item-price">RWF {Math.round(item.unitPrice).toLocaleString()} each</div>
                </div>
                ...
                <span style={{ fontWeight: 700, minWidth: 80, textAlign: 'right' }}>RWF {Math.round(item.unitPrice * item.quantity).toLocaleString()}</span>
          ...
          <div className="pos-cart-footer">
            ...
            <button className="btn btn-primary w-full" style={{ width: '100%' }} disabled={cart.length === 0 || submitting} onClick={completeSale}>
