  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 60, flexDirection: 'column', gap: 16 }}>
        ...
        <p style={{ color: 'var(--text-muted)', margin: 0 }}>Loading batches...</p>
      </div>
    );
  }
    ...
    <div>
      <div className="page-header">
        ...
        {hasPermission('CreateBatches') && (
          <Link to="/dashboard/batches/new" className="btn btn-primary"><Plus size={18} /> Add Batch</Link>
        )}
      </div>
      ...
      <div className="table-container">
        <table className="data-table">
          <thead>
            <tr>
              <th>Batch #</th><th>Medicine</th><th>Qty</th><th>Remaining</th>
              <th>Cost Price</th><th>Expiry Date</th><th>Branch</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center text-muted" style={{ padding: 40 }}>No batches found</td></tr>
            ) : paginated.map(b => {
              const med = b.medicine || medicines.find(m => m.id === b.medicineId);
              const branch = b.branch || branches.find(br => br.id === b.branchId);
              return (
                <tr key={b.id}>
