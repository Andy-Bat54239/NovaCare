...
import { getConversations, getMessages, getChatCustomers } from '../../api/chat';
import {
  getStaffConversations, openStaffConversation,
  getStaffMessages, getColleagues,
} from '../../api/staffChat';
...
function CustomerChatsPanel({ currentUser, connectionRef }) {
  ...
  useEffect(() => {
    loadConversations();
    getChatCustomers().then(setCustomers).catch(() => {});
    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 20, height: 580 }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
              <input className="form-input" placeholder="Search..." value={search}
                onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 30, height: 34, fontSize: '0.83rem' }} />
            </div>
            <button className="btn btn-primary" onClick={() => setShowCustomers(true)}
              style={{ padding: '6px 10px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.8rem' }}>
              <UserPlus size={14} />
            </button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: '0.83rem', margin: 0 }}>No customer conversations</p>
              </div>
            ) : (
              filtered.map(conv => (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontWeight: 600, fontSize: '0.87rem' }}>{conv.customerName}</span>
                        <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>
                          {conv.lastMessageAt ? new Date(conv.lastMessageAt).toLocaleDateString() : ''}
                        </span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}>
                        <span style={{ fontSize: '0.76rem', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 160 }}>
                          {conv.lastMessage || 'No messages yet'}
                        </span>
                        {conv.unreadCount > 0 && (
                          <span style={{ minWidth: 18, height: 18, borderRadius: 999, background: 'var(--primary)', color: 'white', fontSize: '0.68rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px', flexShrink: 0 }}>
        ...
        {activeConvId ? (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{activeConv?.customerName}</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{activeConv?.customerEmail}</div>
              </div>
            </div>
          ...
          <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.875rem', marginTop: 4 }}>Choose a conversation from the left to start messaging</p>
            </div>
          </div>
        )}
      </div>
      ...
      {showCustomers && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#ffffff', borderRadius: 20, padding: '28px 28px 24px', width: 460, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.35)' }}>
            <h3 style={{ margin: '0 0 6px', fontSize: '1.2rem', fontWeight: 800, color: '#0f172a' }}>Start Conversation</h3>
            <p style={{ margin: '0 0 18px', fontSize: '0.83rem', color: '#64748b' }}>Select a customer to begin chatting</p>

            {customers.length === 0 ? (
              <p style={{ color: '#64748b', textAlign: 'center', padding: 24 }}>No registered customers yet.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto', paddingRight: 4 }}>
                {customers.map(c => (
                  <button key={c.id} onClick={() => handleInitiateChat(c)}
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#0f172a' }}>{c.firstName} {c.lastName}</div>
...
function TeamChatPanel({ currentUser, connectionRef }) {
  ...
  const loadConversations = useCallback(() => {
    getStaffConversations().then(setConversations).catch(() => {});
  }, []);

  useEffect(() => {
    loadConversations();
    getColleagues().then(setColleagues).catch(() => {});
  }, [loadConversations]);
  ...
  useEffect(() => {
    ...
    startConnection().then(conn => {
      if (!mounted) return;
      conn.on('ReceiveStaffMessage', (msg) => {
        if (!mounted) return;
  ...
  useEffect(() => {
    if (activeConvId) {
      getStaffMessages(activeConvId).then(setMessages).catch(() => {});
    } else {
      setMessages([]);
    }
  }, [activeConvId]);
  ...
  const handleSend = async (e) => {
    ...
    setSending(true);
    try {
      await conn.invoke('SendStaffMessage', activeConvId, input.trim());
      setInput('');
    } catch { /* silent */ } finally { setSending(false); }
  };
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 20, height: 580 }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
              <input className="form-input" placeholder="Search colleagues..." value={search}
                onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 30, height: 34, fontSize: '0.83rem' }} />
            </div>
            <button className="btn btn-primary" onClick={() => setShowColleagues(true)}
              style={{ padding: '6px 10px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.8rem' }}>
              <UserPlus size={14} />
            </button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: '0.83rem', margin: 0 }}>No team conversations yet</p>
                <p style={{ fontSize: '0.78rem', marginTop: 4, opacity: 0.7 }}>Click + to message a colleague</p>
              </div>
            ) : (
              filtered.map(conv => (
                <button key={conv.id} onClick={() => openConversation(conv)}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Avatar name={conv.otherName} size={36} role={conv.otherRole} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontWeight: 600, fontSize: '0.87rem' }}>{conv.otherName}</span>
                        <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>
                          {conv.lastMessageAt ? new Date(conv.lastMessageAt).toLocaleDateString() : ''}
                        </span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{
                            fontSize: '0.68rem', fontWeight: 600, padding: '1px 6px', borderRadius: 999,
                            background: `${ROLE_COLORS[conv.otherRole]}18`, color: ROLE_COLORS[conv.otherRole],
                          }}>
                            {ROLE_LABELS[conv.otherRole]}
                          </span>
                          <span style={{ fontSize: '0.76rem', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 100 }}>
                            {conv.lastMessage || 'No messages yet'}
                          </span>
                        </div>
                        {conv.unreadCount > 0 && (
        ...
        {activeConvId ? (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 12 }}>
              <Avatar name={activeOther?.name} size={38} role={activeOther?.role} />
              <div>
                <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{activeOther?.name}</div>
                <span style={{
                  fontSize: '0.75rem', fontWeight: 600, padding: '2px 8px', borderRadius: 999,
                  background: `${ROLE_COLORS[activeOther?.role]}18`, color: ROLE_COLORS[activeOther?.role],
                }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
              {messages.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 32 }}>
                  <MessageSquare size={32} style={{ opacity: 0.3, marginBottom: 10 }} />
                  <p style={{ fontSize: '0.875rem' }}>Say hello to your colleague!</p>
                </div>
              )}
              {messages.map((msg, idx) => {
                const isMe = msg.senderId === currentUser?.id;
            ...
            <form onSubmit={handleSend} style={{ padding: '14px 20px', borderTop: '1px solid var(--border-color)', display: 'flex', gap: 10 }}>
              <input className="form-input" placeholder="Message your colleague..." value={input}
                onChange={e => setInput(e.target.value)} style={{ flex: 1 }} />
              <button type="submit" className="btn btn-primary" disabled={!input.trim() || sending} style={{ padding: '10px 16px', flexShrink: 0 }}>
                <Send size={16} />
              </button>
          ...
          <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.875rem', marginTop: 4 }}>Select a colleague or click + to start a new chat</p>
            </div>
          </div>
        )}
      </div>
      ...
      {showColleagues && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#ffffff', borderRadius: 20, padding: '28px 28px 24px', width: 460, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.35)' }}>
            <h3 style={{ margin: '0 0 6px', fontSize: '1.2rem', fontWeight: 800, color: '#0f172a' }}>Message a Colleague</h3>
            <p style={{ margin: '0 0 18px', fontSize: '0.83rem', color: '#64748b' }}>Select a staff member to start a private chat</p>
            {colleagues.length === 0 ? (
              <p style={{ color: '#64748b', textAlign: 'center', padding: 24 }}>No other staff members found.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto', paddingRight: 4 }}>
                {colleagues.map(c => (
                  <button key={c.id} onClick={() => handleOpenConversation(c)}
                    onMouseEnter={e => e.currentTarget.style.background = '#f0fdf9'}
                    onMouseLeave={e => e.currentTarget.style.background = '#f8fafc'}
                    style={{ padding: '14px 16px', borderRadius: 12, border: '1.5px solid #e2e8f0', background: '#f8fafc', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 14, transition: 'background 0.15s' }}>
                    <Avatar name={`${c.firstName} ${c.lastName}`} size={42} role={c.role} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#0f172a' }}>{c.firstName} {c.lastName}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
                        <span style={{ fontSize: '0.72rem', fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: `${ROLE_COLORS[c.role]}18`, color: ROLE_COLORS[c.role] }}>
                          {ROLE_LABELS[c.role]}
                        </span>
                        {c.branch && <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{c.branch}</span>}
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <button onClick={() => setShowColleagues(false)}
              style={{ marginTop: 20, width: '100%', padding: '11px', borderRadius: 10, border: '1.5px solid #e2e8f0', background: '#f1f5f9', color: '#374151', fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer' }}>
              Cancel
            </button>
          </div>
        </div>
    ...
    <div className="dashboard-page">
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid var(--border-color)', marginBottom: 24 }}>
        {tabs.map(({ key, label, icon }) => {
          const TabIcon = icon;
          return <button key={key} onClick={() => setActiveTab(key)}
            style={{
      ...
      {activeTab === 'customers'
        ? <CustomerChatsPanel currentUser={currentUser} connectionRef={connectionRef} />
        : <TeamChatPanel      currentUser={currentUser} connectionRef={connectionRef} />
      }
    </div>
  );
}
